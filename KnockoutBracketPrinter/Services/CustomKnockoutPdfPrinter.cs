...
public class CustomKnockoutPdfPrinter : IBracketPdfPrinter
{
    ...
    private readonly IBracketPageLayoutService _layout;
    private readonly IFlagImageCache _flagCache;
    private Dictionary<string, byte[]> _flagImages = new(StringComparer.OrdinalIgnoreCase);

    public CustomKnockoutPdfPrinter(IBracketPageLayoutService layout, IFlagImageCache flagCache)
    {
        _layout = layout;
        _flagCache = flagCache;
    }

    public async Task GenerateAsync(Bracket bracket, string outputPath)
    {
        QuestPDF.Settings.License = LicenseType.Community;

        var pages = _layout.BuildPages(bracket);
        var totalPages = pages.Count;

        _flagImages = await PreloadFlagsAsync(bracket);
    ...
    private async Task<Dictionary<string, byte[]>> PreloadFlagsAsync(Bracket bracket)
    {
        var result = new Dictionary<string, byte[]>(StringComparer.OrdinalIgnoreCase);

        var urls = bracket.Rounds
            .SelectMany(r => r.Matches)
            .SelectMany(m => new[] { m.Player1?.FlagUrl, m.Player2?.FlagUrl })
        ...
        {
            var bytes = await _flagCache.GetAsync(url);
    ...
    bool isWinner)
    {
        ...
        if (!string.IsNullOrWhiteSpace(player?.FlagUrl) &&
            _flagImages.TryGetValue(player.FlagUrl, out var cachedBytes))
        {
            flagBytes = cachedBytes;
        }
